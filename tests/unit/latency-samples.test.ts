...
beforeEach(() => {
  // Isolate each test — the module-level db handle is shared per worker.
  getDbHandle().prepare(`DELETE FROM latency_samples`).run()
})

describe("persistent execution-latency samples", () => {
  it("inserts a sample and returns it in the recent list", () => {
    insertLatencySample({
      mode: "PAPER_V1", marketId: "mkt-1", exchangeOrderId: "ord-A",
      side: "UP", shares: 10, limitPrice: 0.9,
      quoteAgeMs: 12, decisionMs: 3, preSubmitMs: 7,
      submitMs: 22, fillCheckMs: 18, totalMs: 62, submitAtMs: Date.now(),
    })
    flushWriteQueueSync()
  ...
  it("computes p50/p95/max percentiles across the window", () => {
    ...
    for (let i = 1; i <= 20; i++) {
      insertLatencySample({
        ...
        submitMs: i * 10, fillCheckMs: 0, totalMs: i * 10, submitAtMs: now,
      })
    }
    flushWriteQueueSync()
  ...
  it("second fill observation for the same order id is a no-op", () => {
    ...
    flushWriteQueueSync()
    recordLatencyFillObserved("ord-idem", 0.9, submitAtMs + 999)
    flushWriteQueueSync()
    const [row] = getLatencySamples("LIVE_V2", 1)
